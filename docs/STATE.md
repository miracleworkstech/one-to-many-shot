...
| 16 Copy (D26) | PR open, chained on task/15 | task/16-copy | `/impeccable clarify`: refusals, "on the way", download not export, status-page links; copy only, check green, no evaluator round |
| 17 Navigation flicker, replaying marks, static ring (D24 amended) | PR open, chained on task/16 | task/17-nav-flicker | `next/link` back, cross-fade removed, marks gated on `justDecided`, ring turns; check green |
